...
public class 法令OffGcd : IDecisionResolver
{
    public CheckResult Check()
    {
        ...
        if (WHMSkill.法令.GetActionCooldown() != 0) return new CheckResult(false, "未冷却");
        if (!PromeSettings.Instance.GetQt(WHMQt.法令)) return new CheckResult(false, "未开启QT");
    ...
    {
        return new PAction(WHMSkill.法令, ActionType.OffGcd, ActionTargetType.Self);
    }
}
