...
namespace MilkVio.Tank.PLD.Timeline;

public sealed class PLDAutoIronAction
    : IAction, ISerializableAction, IJobNodeDescriptor
{
    ...
    // ── IJobNodeDescriptor ────────────────────────────────────────
    public string NodeDisplayName => "自动盾姿";
    ...
    public void Execute()
    {
        ...
        if (_enable)
        {
            ...
            {
                var action = new PAction(PLDSkill.钢铁信念, ActionType.OffGcd, ActionTargetType.Self);
                if (_forceUse)
        ...
        else
        {
            if (player.HasStatus(PLDBuff.钢铁信念buff))
            {
                var action = new PAction(PLDSkill.解除钢铁信念, ActionType.OffGcd, ActionTargetType.Self);
                if (_forceUse)
                    ActionUpdater.UseAction(action);
                else
                    ActionQueueManager.Enqueue(action);
    ...
    {
        ActionFactory.Register(context, "pldautoiron", dto =>
        {
            var a = new PLDAutoIronAction();
