...
public sealed class DRKAutoMaliceAction
    : IAction, ISerializableAction, IJobNodeDescriptor
{
    ...
    // ── IJobNodeDescriptor ────────────────────────────────────────
    public string NodeDisplayName => "自动深恶痛绝";
    ...
    public void Execute()
    {
        ...
        if (_enable)
        {
            ...
            {
                var action = new PAction(DRKSkill.深恶痛绝, ActionType.OffGcd, ActionTargetType.Self);
                if (_forceUse)
        ...
        else
        {
            if (player.HasStatus(DRKBuff.深恶痛绝))
            {
                var action = new PAction(DRKSkill.解除深恶痛绝, ActionType.OffGcd, ActionTargetType.Self);
                if (_forceUse)
                    ActionUpdater.UseAction(action);
                else
                    ActionQueueManager.Enqueue(action);
    ...
    {
        ActionFactory.Register(context, "drkautomalice", dto =>
        {
            var a = new DRKAutoMaliceAction();
