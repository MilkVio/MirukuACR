...
public class 短兵相接OffGcd : IDecisionResolver
{
    public CheckResult Check()
    {
        if (!PromeSettings.Instance.GetQt(RDMQt.短兵相接)) return new CheckResult(false, "未开启短兵相接");
        if (ActionHelper.GetGcdRemain() < 1f) return new CheckResult(false, "未在GCD前半");
        ...
        if (RDMSkill.短兵相接.GetActionCharges() >= 1) return new CheckResult(true, "打一个");
    ...
    {
        return new PAction(RDMSkill.短兵相接, ActionType.OffGcd, ActionTargetType.Target);
    }
}
