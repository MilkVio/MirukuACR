...
public class 荆棘环绕OffGcd : IDecisionResolver
{
    public CheckResult Check()
    {
        ...
        // 自身不可以的条件
        if (RDMSkill.荆棘环绕.GetActionCooldown() != 0) return new CheckResult(false, "未冷却");
        if (me.HasStatus(RDMBuff.荆棘环绕预备)) return new CheckResult(true, "打一个");
    ...
    {
        return new PAction(RDMSkill.荆棘环绕, ActionType.OffGcd, ActionTargetType.Target);
    }
}
