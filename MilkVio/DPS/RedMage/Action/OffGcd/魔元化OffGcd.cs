...
public class 魔元化OffGcd : IDecisionResolver
{
    public CheckResult Check()
    {
        ...
        var isCan120Use = cd120 < 120 && cd120 > 100;
        if (RedMageHelper.GetNextManaActionPhase(false) > 1) return new CheckResult(false, "三连中不准按");

        if (PromeSettings.Instance.GetQt(RDMQt.倾泻资源))
        {
            if (RDMSkill.魔元化.GetActionCooldown() == 0) return new CheckResult(true, "冷却了打掉");
        }
        if (RDMSkill.魔元化.GetActionCooldown() == 0 && isCan120Use) return new CheckResult(true, "冷却了打掉");
    ...
    {
        return new PAction(RDMSkill.魔元化, ActionType.OffGcd, ActionTargetType.Target);
    }
}
