...
public class 剑舞Gcd : IDecisionResolver //12 强化12 大舞小舞结束动作落幕舞
{
    // 检查什么时候可以用这个技能
    ...
    public CheckResult Check()
    {
        ...
        if (Core.Me.DistanceToMe() <= gcd和目标距离)
        {
            if (伶俐 >= 85)
            {
                return new CheckResult(true, " 或者伶俐>=80");
            }

            if (伶俐 >= 50 && (me.HasStatus(DNCBuff.技巧舞步结束)||PromeSettings.Instance.GetQt(DNCQt.倾泻资源)))
            {
                return new CheckResult(true, "120或倾泻资源");
            }
        }
