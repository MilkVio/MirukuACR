...
public class 动物彩绘_立即Gcd : IDecisionResolver
{
    public CheckResult Check()
    {
        // QT控制
        if (JobGaugeHelper.PCT.CreatureMotifDrawn) return new CheckResult(false, "当前拥有动物画");
        if (!PromeSettings.Instance.GetQt(PCTQt.动物画)) return new CheckResult(false, "未开启动物画");
        ...
        if (canUse)
        {
            ...
            if (PromeSettings.Instance.GetQt(PCTQt.立即动物画))
            {
