...
public class 六道轮回OffGcd : IDecisionResolver
{
    public CheckResult Check()
    {
        ...
        if(NinjaSkill.六道轮回.GetActionCooldown() > 0) return new CheckResult(false, "未冷却");
        
        var ninki = JobGaugeHelper.NIN.Ninki;

        if (ninki >= 50)
        {
            if (PromeSettings.Instance.GetQt(NinjaQt.分身之术) && NinjaSkill.分身之术.GetActionCooldown() < 10)
            {
                if (ninki >= 90) return new CheckResult(true, "分身前要溢出了 打一个");
                return new CheckResult(false, "先打分身");
            }
            if (ninki >= 80)
            {
    ...
    {
        return new PAction(NinjaSkill.六道轮回, ActionType.OffGcd, ActionTargetType.Target);
    }
}
